...
class UserDB:
    ...
    async def update_email(self, user_id: int, email: str) -> None:
        async with self.db_pool.get_connection() as session:
            stmt = update(User).where(User.id == user_id).values(email=email) # type: ignore
            await session.execute(stmt)
            await session.commit()

    async def update_full_name(self, user_id: int, full_name: str) -> None:
        async with self.db_pool.get_connection() as session:
            stmt = update(User).where(User.id == user_id).values(full_name=full_name) # type: ignore
            await session.execute(stmt)
            await session.commit()

    async def update_is_active(self, user_id: int, is_active: bool) -> None:
        async with self.db_pool.get_connection() as session:
            stmt = update(User).where(User.id == user_id).values(is_active=is_active) # type: ignore
            await session.execute(stmt)
            await session.commit()

    async def update_password(self, user_id: int, hashed_password: str) -> None:
        async with self.db_pool.get_connection() as session:
            stmt = update(User).where(User.id == user_id).values(hashed_password=hashed_password) # type: ignore
            await session.execute(stmt)
            await session.commit()
